...
abstract class BasicTableGateway
{
    ...
    public function getExerciseWithMostWeight(int $user_id, int $exercise_id): array
    {
        $sql = $this->connection->prepare("SELECT * FROM progress WHERE user_id = ? AND exercise_id = ? ORDER BY weight DESC LIMIT 1");
        $sql->execute([$user_id, $exercise_id]);

        $progressData = $sql->fetch(PDO::FETCH_ASSOC);
    ...
    public function findByFields(array $fields, string $orderBy = ''): array|false
{
